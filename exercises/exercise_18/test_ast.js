...
describe("Class Constructor Parsing Tests", function () {
  it("Класс Computer должен быть корректно обработан", function () {
    ...
    assert.strictEqual(computerClass.id.modificator, "private", "Модификатор id должен быть private");
    assert.strictEqual(computerClass.name.modificator, "protected", "Модификатор name должен быть protected");
    assert.strictEqual(computerClass.model.modificator, "opened", "Модификатор model должен быть public");
  });
  ...
  it("Конструкторы должны иметь правильные сигнатуры", function () {
    ...
    assert.ok(computerClass.constructorSignature0, "Первая сигнатура конструктора не найдена");
    assert.ok(computerClass.constructorSignature1, "Вторая сигнатура конструктора не найдена");
    assert.ok(computerClass.constructor, "Основная реализация конструктора не найдена");
    ...
    assert.strictEqual(mainConstructor.params.length, 4, "Должно быть 4 параметра");
    assert.strictEqual(
